...
export default class AddPicScreen extends React.Component {
  ...
  confirm = () => {
    ...
    var ImageList = this.props.navigation.state.params.newVar.ImageList
    // data[name] = formValues
    // if(!(ImageList.indexOf(name) >= 0)){
    //   ImageList.push(name);
    // }
    ImageList.push(this.state.previewImageData)
    ...
    this.props.navigation.navigate('AutoFill', {newVar})
  }
  ...
  takePicture = async function() {
    if(this.camera){
      ...
      this.state.previewImageData = 'data:image/gif;base64,'+photo.base64;
      this.state.enablePreview = true;
      this.forceUpdate();
    }
  }
